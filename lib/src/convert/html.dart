...
//      container = getContainer(op.attributes);
      if (lf == -1) {
        _handleSpan(removeZeroWidthSpace((op.data as String)), op.attributes);
      } else {
  ...
  void _writeEmbedTag(
      StringBuffer buffer, NotusAttribute<dynamic> embed,
      {bool close = false}) {
  ...
  void _writeLinkTag(StringBuffer buffer, NotusAttribute<dynamic> link,
      {bool close = false}) {
    if (close) {
      buffer.write('</${htmlKeys.anchor}>');
    } else {
      buffer.write(
          '<${htmlKeys.anchor} ${htmlKeys.anchorHref}=\"${link.value as String}\"${buildContainer(deltaKeys.a)}>');
    }
  }

  void _writeHeadingTag(StringBuffer buffer, NotusAttribute<dynamic> heading,
      {bool close = false}) {
  ...
  void _writeBlockTag(StringBuffer buffer, NotusAttribute<dynamic> block,
...
class _HTMLNotusDecoder extends Converter<String, Delta> {
    ...
    for (final elem in elemStack) {
      switch (elem.localName) {
        ...
        case htmlKeys.horizontalRule:
          ...
          break;
        case htmlKeys.list:
          if (elem.parent != null && elem.parent!.localName == htmlKeys.orderedList) {
            deltaAttributeLine[deltaKeys.block] = deltaKeys.ol;
          } else if (elem.parent != null && elem.parent!.localName == htmlKeys.unorderedList) {
  ...
  List<Map<String, dynamic>> toDeltaFormatList(Element element) {
    ...
    void insert(idx, String text, elemStack) {
      ...
      if (text.isEmpty && (attrInline != null && attrInline.isEmpty) && (attrLine != null && attrLine.isEmpty)) return;
      final int originalLength = deltaFormatList.length;
      int shiftIdx() => idx + deltaFormatList.length - originalLength;
      void insertText(txt) {
        if (txt.isNotEmpty) {
          if (attrInline != null && attrInline.isEmpty) {
        ...
        } else if (attrInline != null && attrInline.containsKey("embed")) {
          deltaFormatList.insert(shiftIdx(), {
            deltaKeys.insert: String.fromCharCode(8203),
            deltaKeys.attributes: attrInline,
          });
        }
        if (attrLine != null && attrLine.isNotEmpty &&
            (txt.isNotEmpty || attrInline != null && attrInline.containsKey("embed"))) {
          deltaFormatList.insert(shiftIdx(), {
            deltaKeys.insert: "\n",
      ...
      if (attrLine != null && attrLine.containsKey("block")) {
